...
const TabButton = ({ icon: Icon, label, active }) => (
  <button className={`flex flex-col items-center justify-center flex-1 py-2 rounded-xl transition-all ${
...
const SecureNavBar = ({ active = 'Home' }) => {
  return (
    <nav className="fixed bottom-4 left-1/2 -translate-x-1/2 w-[92%] max-w-md bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border border-slate-200 dark:border-slate-800 rounded-2xl p-2 shadow-xl">
      <div className="flex gap-2">
        ...
        <TabButton icon={Map} label="Zones" active={active === 'Zones'} />
        <TabButton icon={BarChart2} label="Logs" active={active === 'Logs'} />
        <TabButton icon={Users} label="Users" active={active === 'Users'} />
        <TabButton icon={Settings} label="Settings" active={active === 'Settings'} />
      </div>
    </nav>
  );
};
